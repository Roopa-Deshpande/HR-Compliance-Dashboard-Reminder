...
export async function toggleRecordDone(id, type, fields, doneVal, summary) {
  await updateDoc(doc(db, "records", id), {
    done: doneVal, updatedAt: serverTimestamp(), updatedBy: currentUser?.name || "Unknown"
  });
  await logAction({
    action: "Edit", recordType: type, recordId: id, recordSummary: summary,
    previousValue: { done: !doneVal }, newValue: { done: doneVal }
  });
}
